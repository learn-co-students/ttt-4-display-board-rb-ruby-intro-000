...
describe "#display_board in 'lib/display_board.rb" do
  context 'various game situations' do
    ...
    it 'prints an entire board full of Os' do
      board = ["O", "O", "O", "O", "O", "O", "O", "O", "O"]# Can you copy the syntax of the tests above to write a test for a board
      output = capture_puts{ display_board(board) } if defined?(display_board)
      ...
      expect(rows[3]).to eq("-----------")
      expect(rows[4]).to eq(" O | O | O ")# Hint: You should be able to copy the code in the previous it example
