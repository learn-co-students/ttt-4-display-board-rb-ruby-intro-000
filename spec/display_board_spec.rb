...
describe "#display_board in 'lib/display_board.rb" do
  context 'various game situations' do
    ...
    it 'prints an entire board full of Xs' do
      ...
      # *** Edit the line below ***
      board = ["X", "X", "X", "X", "X", "X", "X", "X", "X"] # This is not correct - it is now. -CH
    ...
    it 'prints a board of arbitrary square size' do
      board = ["X", "X", "X", "X", "X", "X", "X", "X", "O", "O", "O", "O", "O", "O", "O", "O"] # 4 x 4 board
      output = capture_puts{ display_board(board) } if defined?(display_board)
      rows = output.split("\n")

      expect(rows[0]).to eq(" X | X | X | X ")
      expect(rows[1]).to eq("---------------")
      expect(rows[2]).to eq(" X | X | X | X ")
      expect(rows[3]).to eq("---------------")
      expect(rows[4]).to eq(" O | O | O | O ")
      expect(rows[5]).to eq("---------------")
      expect(rows[6]).to eq(" O | O | O | O ")
